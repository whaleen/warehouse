...
**CRITICAL**: GE SSO credentials are stored IN Supabase `settings` table, NOT in environment variables. This allows per-location credentials and keeps sensitive data out of code.
...
GE SSO credentials are stored in the `settings` table:
...
Insert/update in `settings` table:
...
INSERT INTO settings (id, sso_username, sso_password)
VALUES (
...
**Same as local** - credentials are in Supabase `settings` table.
...
3. Fetch GE SSO credentials from `settings` table
...
- [ ] GE SSO credentials in `settings` table
...
✅ Store GE SSO credentials in Supabase `settings` table
...
### For Developers
- Use this for configuring GE sync locally and in production.
- Keep the `settings` table updated per location.

### For Operators
- This is IT/deployment documentation, not a daily workflow guide.

### For Agent
- Do not expose secrets; use this only for setup guidance.
```
